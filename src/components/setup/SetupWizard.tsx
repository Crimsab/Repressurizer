...
export function SetupWizard() {
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      setError(t("setup.error.detectFailed", { error: String(e) }));
    } finally {
      setLoading(false);
    }
  };
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setError(t("setup.error.invalidPath", { error: String(e) }));
                    } finally {
                      setLoading(false);
                    }
                  }}
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/8 px-4 py-3">
                <p className="text-sm font-medium text-amber-300">{t("setup.safety")}</p>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-repressurizer-border-subtle bg-repressurizer-bg px-4 py-3">
                  <p className="text-[11px] uppercase tracking-wider text-repressurizer-text-faint font-medium">{t("setup.summary.games")}</p>
                  <p className="mt-1 font-mono text-2xl font-semibold text-white tabular-nums">{loadedGames.length}</p>
                </div>
                <div className="rounded-xl border border-repressurizer-border-subtle bg-repressurizer-bg px-4 py-3">
                  <p className="text-[11px] uppercase tracking-wider text-repressurizer-text-faint font-medium">{t("setup.summary.collections")}</p>
              ...
              <div className="rounded-xl border border-repressurizer-border-subtle bg-repressurizer-bg px-4 py-3 text-xs leading-relaxed text-repressurizer-text-muted">
                <p>{t("setup.summary.backups")}</p>
                <p className="mt-1">{t("setup.summary.closeSteam")}</p>
              </div>
