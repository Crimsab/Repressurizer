...
export function OnboardingTour({ onComplete }: OnboardingTourProps) {
  ...
  const runFamilyProbe = async (fromAuto = false) => {
    ...
    setFamilyMessage(fromAuto ? t("onboarding.family.checkingSaved") : t("onboarding.family.checking"));
    ...
    try {
      ...
      if (!settings.apiKey && !accessToken) {
        ...
        setFamilyMessage(t("onboarding.family.tokenNeeded"));
        return;
      }
      ...
      if (!extractStoreWebApiToken(familyToken)) {
        setFamilyStatus("needs-token");
        setFamilyMessage(t("onboarding.family.apiRejected"));
        return;
      }

      setFamilyStatus("error");
      setFamilyMessage(t("onboarding.family.failed", { error: text }));
    }
  };
  ...
  const handleFamilyTokenPaste = (event: ClipboardEvent<HTMLInputElement>) => {
    ...
    if (token && token !== pasted.trim()) {
      ...
      setFamilyMessage(t("settings.family.tokenExtracted"));
    }
  };
          ...
          <div className="mb-6 rounded-xl border border-repressurizer-border-subtle bg-repressurizer-bg px-4 py-3 text-left">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs leading-relaxed text-repressurizer-text-faint">
                  {familyMessage || t("onboarding.family.fallback")}
                </p>
              </div>
            </div>
            ...
            {familyResult && (
              <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
                <MiniFamilyStat label={t("settings.family.shared")} value={familyResult.shared_apps} />
                <MiniFamilyStat label={t("settings.family.owned")} value={familyResult.owned_apps} />
                <MiniFamilyStat label={t("sidebar.hidden")} value={familyResult.non_game_apps} />
              </div>
            )}
